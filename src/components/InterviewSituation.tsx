...
import { AlertTriangle, Users, MapPin, Target, CheckCircle, XCircle, Cpu, Wrench } from 'lucide-react';
...
const competitionRangeData = [{
  ...
  range: '8–12 candidates per core opening',
  color: 'blue'
}, {
  role: 'Avionics Engineer',
  range: '5–8 candidates per core opening',
  color: 'purple'
}, {
  role: 'AME',
  range: '3–6 candidates per licensed opening',
  color: 'amber'
}];
...
const aerospaceEngineerCityData = [{
  city: 'Bengaluru',
  range: '10–14',
  meaning: 'Largest hub, toughest competition'
}, {
  city: 'Hyderabad',
  range: '6–9',
  meaning: 'Best balance for freshers'
}, {
  city: 'Pune',
  range: '7–10',
  meaning: 'Manufacturing + design roles'
}, {
  city: 'Chennai',
  range: '6–9',
  meaning: 'Stable aerospace growth'
}, {
  city: 'Nagpur',
  range: '4–7',
  meaning: 'Easier entry, MRO/manufacturing'
}];
...
const avionicsEngineerCityData = [{
  city: 'Bengaluru',
  range: '8–12',
  meaning: 'High-end avionics & defence work'
}, {
  city: 'Hyderabad',
  range: '5–8',
  meaning: 'Growing avionics ecosystem'
}, {
  city: 'Pune',
  range: '6–9',
  meaning: 'Embedded & testing roles'
}, {
  city: 'Chennai',
  range: '5–8',
  meaning: 'Steady avionics hiring'
}, {
  city: 'Kochi',
  range: '4–6',
  meaning: 'Naval & maintenance avionics'
}];
...
const ameCityData = [{
  city: 'Delhi-NCR',
  range: '5–7',
  meaning: 'High traffic, more licensed AMEs'
}, {
  city: 'Mumbai',
  range: '4–6',
  meaning: 'Busy MRO hub, steady demand'
}, {
  city: 'Hyderabad',
  range: '3–5',
  meaning: 'Less crowded, good entry chances'
}, {
  city: 'Bengaluru',
  range: '4–6',
  meaning: 'Mixed civil + defence work'
}, {
  city: 'Nagpur',
  range: '2–4',
  meaning: 'Strong MRO demand, low crowd'
}];
        ...
        <CardContent>
          ...
          <div className="hidden md:block overflow-x-auto">
            <div className="rounded-xl overflow-hidden border border-slate-200">
              ...
              {competitionRangeData.map((item, index) => <div key={index} className={`grid grid-cols-2 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50'} hover:bg-blue-50 transition-colors border-b border-slate-100 last:border-b-0`}>
                  <div className="p-4 flex items-center gap-3">
                    <span className={`px-3 py-1.5 rounded-full text-xs font-semibold ${item.color === 'blue' ? 'bg-blue-100 text-blue-800' : item.color === 'purple' ? 'bg-purple-100 text-purple-800' : 'bg-amber-100 text-amber-800'}`}>
                      {item.role}
                    </span>
                  </div>
          ...
          <div className="md:hidden space-y-4">
            {competitionRangeData.map((item, index) => <div key={index} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="hidden md:block overflow-x-auto">
            <div className="rounded-xl overflow-hidden border border-blue-200">
              ...
              {aerospaceEngineerCityData.map((item, index) => <div key={index} className={`grid grid-cols-3 ${index % 2 === 0 ? 'bg-white' : 'bg-blue-50/50'} hover:bg-blue-100/50 transition-colors border-b border-blue-100 last:border-b-0`}>
          ...
          {/* Mobile Cards */}
          <div className="md:hidden space-y-4">
            {aerospaceEngineerCityData.map((item, index) => <div key={index} className="bg-white p-4 rounded-xl border border-blue-200 shadow-sm">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="hidden md:block overflow-x-auto">
            <div className="rounded-xl overflow-hidden border border-purple-200">
              ...
              {avionicsEngineerCityData.map((item, index) => <div key={index} className={`grid grid-cols-3 ${index % 2 === 0 ? 'bg-white' : 'bg-purple-50/50'} hover:bg-purple-100/50 transition-colors border-b border-purple-100 last:border-b-0`}>
                  <div className="p-4 flex items-center gap-2">
          ...
          {/* Mobile Cards */}
          <div className="md:hidden space-y-4">
            {avionicsEngineerCityData.map((item, index) => <div key={index} className="bg-white p-4 rounded-xl border border-purple-200 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  <MapPin className="w-4 h-4 text-purple-600" />
        ...
        <CardContent className="space-y-6">
          ...
          <div className="hidden md:block overflow-x-auto">
            <div className="rounded-xl overflow-hidden border border-amber-200">
              ...
              {ameCityData.map((item, index) => <div key={index} className={`grid grid-cols-3 ${index % 2 === 0 ? 'bg-white' : 'bg-amber-50/50'} hover:bg-amber-100/50 transition-colors border-b border-amber-100 last:border-b-0`}>
                  <div className="p-4 flex items-center gap-2">
          ...
          {/* Mobile Cards */}
          <div className="md:hidden space-y-4">
            {ameCityData.map((item, index) => <div key={index} className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  <MapPin className="w-4 h-4 text-amber-600" />
      ...
      <Card className="border-2 border-primary/20">
        <CardHeader className="p-4 sm:p-6">
          ...
          <div className="flex items-center justify-center mt-3">
            <div className="bg-amber-50/80 border border-amber-200/60 rounded-xl px-3 py-2 sm:px-4 sm:py-2.5">
              <p className="text-amber-700 text-[0.6875rem] sm:text-xs md:text-sm text-center">
                <span className="font-semibold">Why this matters:</span> Helps students pause, self-check comfort level, and take the next step calmly before committing to a high-stakes career decision.
              </p>
            </div>
          </div>
        </CardHeader>
